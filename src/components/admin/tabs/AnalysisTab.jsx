          ...
          <li key={item.id} className="space-y-1">
            <div className="flex items-baseline justify-between gap-3 text-sm">
              <span className="flex min-w-0 items-center gap-2">
                <span className={`inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-[10px] font-bold ring-1 ${curveStyle.chip}`}>
                  {curveStyle.label}
                </span>
                <span className="min-w-0 flex-1 truncate font-medium text-slate-700">
                  <span className="text-slate-400">#{item.rank}</span> {item.name || item.email || item.id}
            ...
            <div className="h-1.5 overflow-hidden rounded-full bg-slate-100">
              <div className={`h-full bg-gradient-to-r ${curveStyle.bar}`} style={{ width: `${Math.max(4, Math.min(100, item.sharePct))}%` }} />
            </div>
          </li>
        );
      })}
...
function CohortHeatmap({ cohorts, months }) {
  if (!cohorts || cohorts.length === 0) {
    return <EmptyState icon="grid-3x3" title="Sem coortes" description="Coorte aparece com pelo menos 1 mês de pedidos aprovados." />;
  }
      ...
      <table className="min-w-full text-xs">
        <thead>
          <tr className="text-slate-500">
            <th className="sticky left-0 z-10 bg-white px-2 py-2 text-left font-semibold uppercase tracking-wide">Coorte</th>
            <th className="px-2 py-2 text-center font-semibold uppercase tracking-wide">Tam.</th>
            {headerOffsets.map((offset) => (
              <th key={offset} className="px-2 py-2 text-center font-semibold">M+{offset}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {cohorts.map((cohort) => (
            <tr key={cohort.cohortMonth} className="border-t border-slate-100">
              <td className="sticky left-0 z-10 bg-white px-2 py-1.5 font-semibold text-slate-700">{cohort.cohortMonth}</td>
              ...
              {headerOffsets.map((offset) => {
                ...
                return (
                  <td key={offset} className="px-1 py-1">
                    <div className={`flex h-7 w-12 items-center justify-center rounded font-semibold ${cellTone(cell.retentionPct)}`} title={`${cell.activeCount}/${cohort.cohortSize} ativos em ${cell.monthKey}`}>
...
export function AnalysisTab({ categories = [] }) {
  ...
  function exportProductsCsv() {
    ...
    downloadCsv({
      ...
      columns: [
        ...
        { key: 'revenue', label: 'Receita', format: (v) => Number(v || 0).toFixed(2).replace('.', ',') },
        { key: 'sharePct', label: '% Share', format: (v) => Number(v || 0).toFixed(2).replace('.', ',') },
        { key: 'cumulativePct', label: '% Acumulado', format: (v) => Number(v || 0).toFixed(2).replace('.', ',') },
      ],
      rows: products.items,
    });
  }
  ...
  function exportCustomersCsv() {
    ...
    downloadCsv({
      ...
      columns: [
        ...
        { key: 'revenue', label: 'Receita', format: (v) => Number(v || 0).toFixed(2).replace('.', ',') },
        { key: 'avgTicket', label: 'Ticket médio', format: (v) => Number(v || 0).toFixed(2).replace('.', ',') },
        { key: 'sharePct', label: '% Share', format: (v) => Number(v || 0).toFixed(2).replace('.', ',') },
      ],
  ...
  function exportCohortCsv() {
    ...
    const maxOffset = Math.max(0, ...cohort.cohorts.flatMap((c) => c.retention.map((r) => r.offset)));
    const columns = [
      ...
      ...Array.from({ length: maxOffset + 1 }, (_, offset) => ({
        key: `m${offset}`,
        label: `M+${offset}`,
        format: (v) => (v === null || v === undefined ? '' : Number(v).toFixed(1).replace('.', ',')),
      })),
    ];
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
        <div>
          <p className="text-xs font-bold uppercase tracking-widest text-slate-500">Curva ABC + Coorte</p>
          <p className="text-sm text-slate-700">Decisões de produto, estoque e campanha baseadas em receita real.</p>
        </div>
        <div className="flex flex-wrap gap-2 text-xs">
          ...
          >
            {PERIOD_OPTIONS.map((opt) => (
              <option key={opt.value} value={opt.value}>Produtos: {opt.label.toLowerCase()}</option>
            ))}
          </select>
          <select
            value={categoryId}
          ...
          >
            {PERIOD_OPTIONS.map((opt) => (
              <option key={opt.value} value={opt.value}>Clientes: {opt.label.toLowerCase()}</option>
            ))}
          </select>
        </div>
      </div>
      ...
      >
        ...
        <ParetoList
          ...
          getMeta={(item) => item.categoryName}
        />
        ...
        <div className="mb-4 grid grid-cols-3 gap-2 text-center">
          {Object.entries(RELATIONSHIP_STYLES).map(([key, style]) => (
            <div key={key} className={`rounded-xl px-3 py-2 ring-1 ${style.chip}`}>
              ...
              <p className="mt-1 text-[10px] font-semibold uppercase tracking-wide">{style.label}</p>
            </div>
          ))}
        </div>
        <ParetoList
          items={customers?.items}
          getMeta={(item) => `${item.orderCount} pedido${item.orderCount === 1 ? '' : 's'} · ticket ${formatPrice(item.avgTicket)}`}
        />
        <p className="mt-3 text-xs text-slate-500">
