...
O custo já está visível: `src/utils/api.js` tem um *shim* em `parseJson` que detecta `error` como
...
Mudança de URL é *breaking*: fazer via rota de compatibilidade, sem pressa.
...
`api/create-payment.js` documenta o buraco na própria linha 15: *"os `max()` do zod em
`validation/payment.schemas.js` NUNCA valeram neste caminho"*.
...
`eslint.config.js` documenta que os registrou como *no-op* justamente para que apareçam e sejam
...
que o limiter de cupom *"só existia no Express de desenvolvimento"* e que **a divergência dev/prod
...
> **Correção da regra (13/08/2026).** A redação original mandava migrar também *"a coluna do
> banco, quando der para migrar"*. A premissa estava errada: as colunas já são
...
inclusive já anuncia que `ProjectDocs/` está *"substituindo os MDs avulsos abaixo"* — falta concluir
