...
const STORAGE_HOST_PATTERN =
  /^https:\/\/[^/]+\.supabase\.co\/storage\/v1\/object\/(?:public\/|sign\/|authenticated\/)?([^/?#]+)\/([^?#]+)/i;
...
function isSafeObjectPath(path) {
  ...
  return variants.every((variant) =>
    variant.split('/').every((segment) => segment !== '' && segment !== '.' && segment !== '..'),
  );
}
...
async function createSignedDownloadUrl(downloadUrl, { ttlSeconds = DEFAULT_TTL_SECONDS } = {}) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      log.warn('sign_falhou', { status: res.status, body: body.slice(0, 200) });
      return null;
    }
    ...
    log.warn('handler_failed', { reason: err.message });
    return null;
  }
}
