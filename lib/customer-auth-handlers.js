...
async function customerLogin(req, res) {
  try {
    ...
    console.error('[auth/customer/login]', error.message);
...
async function customerRegister(req, res) {
  try {
    ...
    if (!response.ok) {
      return res.status(400).json({ success: false, error: mapSupabaseAuthError(data, response.status) });
    }
    ...
    console.error('[auth/customer/register]', error.message);
...
async function customerGoogleStart(req, res) {
  try {
    ...
    console.error('[auth/customer/google/start]', error.message);
...
async function customerGoogleCallback(req, res) {
  try {
    ...
    console.error('[auth/customer/google/callback]', error.message);
