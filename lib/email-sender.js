...
// ════════════════════════════════════════════════════════════════════
// TRANSPORTER ÚNICO POR INSTÂNCIA, COM POOL — §2.2 do doc de otimização.
//
// Antes, `buildTransporter()` era chamado DENTRO de `sendEmail`: cada e-mail
// pagava TCP + handshake TLS + AUTH do zero. Somado aos 3 round-trips ao
// Supabase por mensagem (findExistingSentLog, recordSentLog('queued'),
// updateSentLogStatus('sent')), isso colocava `handlers/cron-email-jobs.js` contra
// o `maxDuration: 60` do vercel.json — e o cron roda de hora em hora. Passando
// do teto a função é morta no meio; a idempotência por (email, kind,
// entity_id) faz a próxima hora retomar, mas a FILA NUNCA ESVAZIA se a taxa de
// entrada superar a de saída. Falha por crescimento, em silêncio, exatamente
// quando a newsletter começa a dar certo.
//
// ── ISTO NÃO VIOLA A REGRA E2 ───────────────────────────────────────
// E2 ("nada de estado em memória dentro de função serverless") existe para
// estado DE DECISÃO — cache de dados, contador de rate limit — cujo acerto
// entre instâncias é acidental e cuja perda produz resposta errada. Um pool de
// conexões é RECURSO DE I/O local à instância: se a instância morre, o pool
// morre junto e nada fica inconsistente. Nenhuma decisão do produto depende
// dele existir.
//
// ⚠️ `maxConnections` começa em 3 e é configurável por env. Estourar o limite
// de conexões simultâneas do provedor troca "lento" por "bloqueado", que é
// pior — confirme o limite do Resend antes de subir.
// ════════════════════════════════════════════════════════════════════
...
/** Só para teste: força a próxima chamada a criar um transporter novo. */
