...
async function updateSentLogStatus(logId, status, error) {
  ...
  try {
    await updateTable('email_sent_log', { id: `eq.${logId}` }, {
      status,
      error: error || null,
      sent_at: status === 'sent' ? new Date().toISOString() : null,
    });
...
async function sendEmail({ to, subject, html, kind, entityId = null, unsubscribeToken = null }) {
  ...
  if (!isSmtpConfigured()) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.log(`[email-sender] SKIP (no SMTP): ${kind} → ${email}`);
    }
  ...
  try {
    ...
    console.error(`[email-sender] FAILED (${kind} → ${email}):`, error.message);
