...
describe('enforceRateLimit', () => {
  ...
  beforeEach(() => {
    ...
    // Evento de segurança com `severity: 'error'` sai por console.error desde
    // que lib/security-logger.js passou a rotear pelo lib/logger.js (regra F1),
    // que mapeia severidade para o stream correto — é o que faz o filtro de
    // erro da Vercel enxergar um fail-open do rate limit. Espiar só o warn
    // deixaria estes testes verdes por não verem o evento.
  ...
  describe('contagem dentro do limite', () => {
    it('libera a requisição e expõe os headers RateLimit-*', async () => {
      rpcResult = {
        kind: 'resolve',
        value: rpcRow({ allowed: true, remaining: 4, retryAfter: 600 }),
      };
    ...
    it('mantém os limites herdados do Express de dev (paridade dev/prod)', () => {
      expect(RATE_LIMITS.adminLogin).toMatchObject({
        bucket: 'admin-login',
        limit: 5,
        windowSeconds: 600,
      });
      expect(RATE_LIMITS.validateCoupon).toMatchObject({
        bucket: 'validate-coupon',
        limit: 20,
        windowSeconds: 60,
      });
      expect(RATE_LIMITS.subscribe).toMatchObject({
        bucket: 'subscribe',
        limit: 5,
        windowSeconds: 60,
      });
      expect(RATE_LIMITS.createPayment).toMatchObject({
  ...
  describe('estouro do limite', () => {
    it('responde 429 com o envelope padrão, Retry-After e blocked=true', async () => {
      ...
      // Envelope da regra A1 com o código da A2 em SCREAMING_SNAKE (P1.5). O
      // código ficava minúsculo e FORA do objeto `error`, então o `parseJson`
      // do cliente devolvia `errorCode: null` e o `if` por RATE_LIMITED do
      // front era um ramo morto.
    ...
    it('registra rate_limit_exceeded com bucket e IP (sem PII do corpo)', async () => {
      rpcResult = {
        kind: 'resolve',
        value: rpcRow({ allowed: false, hitCount: 21, remaining: 0 }),
      };
  ...
  // ── failMode: 'closed' — os baldes que guardam credencial ─────────
  describe('contador indisponível em balde fail-closed', () => {
    // Estes três não podem herdar o fail-open: o serviço que eles protegem
    // (GoTrue, e o anti-replay do TOTP) pode estar de pé com a RPC do contador
    // fora do ar, e aí "passar" é brute force sem teto contra um alvo vivo.
    const FECHADOS = [
      ['customerLogin', RATE_LIMITS.customerLogin],
      ['customerRegister', RATE_LIMITS.customerRegister],
      ['adminLoginSecondFactor', RATE_LIMITS.adminLoginSecondFactor],
    ];

    it.each(FECHADOS)('%s recusa com 503 quando a RPC cai', async (_nome, perfil) => {
      rpcResult = { kind: 'reject', value: new Error('ECONNRESET') };
      const res = createMockRes();

      const gate = await enforceRateLimit(createReq({ 'x-real-ip': '203.0.113.9' }), res, perfil);
    ...
    it('responde no envelope com SERVICE_UNAVAILABLE, não RATE_LIMITED', async () => {
      // 429 diria "você excedeu o limite", que é falso, e mandaria esperar a
      // janela inteira. O código precisa dizer o que de fato houve.
      ...
      expect(String(res.headers['retry-after'])).toBe('30');
    });

    it('recusa também quando o Supabase não está configurado', async () => {
      ...
      expect(gate.blocked).toBe(true);
      expect(gate.reason).toBe('supabase_not_configured');
      expect(res.status).toHaveBeenCalledWith(503);
    });

    it('a recusa continua emitindo o evento de segurança', async () => {
      // Recusar não pode ser mais silencioso do que passar: quem opera precisa
      // ver que o contador está fora, seja qual for a política do balde.
      rpcResult = { kind: 'reject', value: new Error('ECONNRESET') };
    ...
    it('adminLogin segue fail-open — trancar a dona fora durante incidente é pior', async () => {
      rpcResult = { kind: 'reject', value: new Error('ECONNRESET') };
      const res = createMockRes();
  ...
  describe('fail-open em falha de infraestrutura', () => {
    ...
    it('libera quando o PostgREST devolve erro HTTP (ex.: função ausente)', async () => {
      rpcResult = {
        kind: 'http',
        value: { ok: false, status: 404, body: { message: 'function does not exist' } },
      };
  ...
  describe('derivação do identificador', () => {
    ...
    it('cai em socket.remoteAddress e, sem nada, no balde "unknown"', async () => {
      expect(
        await identifierFor(createReq({}, { socket: { remoteAddress: '203.0.113.12' } })),
      ).toBe('203.0.113.12');
      expect(await identifierFor(createReq({}))).toBe('unknown');
    });

    it('normaliza (trim + lowercase) e limita o tamanho da chave', async () => {
      expect(await identifierFor(createReq({ 'x-real-ip': '  2001:DB8::AB  ' }))).toBe(
        '2001:db8::ab',
      );
    ...
    it('aceita identificador explícito no lugar do IP', async () => {
      await enforceRateLimit(createReq({ 'x-real-ip': '203.0.113.9' }), createMockRes(), {
        ...RATE_LIMITS.adminLogin,
  ...
  describe('verify-payment: escopo por order_code + teto de varredura por IP', () => {
    ...
    function verifyReq(orderId, headers = {}) {
      return createReq(
        { 'x-real-ip': IP, ...headers },
        { method: 'POST', body: { orderId, email: 'a@b.com' } },
      );
    }
    ...
    it('lê o order_code também do GET (req.query), que o handler mantém por compatibilidade', async () => {
      ...
      expect(bodiesByBucket()['verify-payment'][0].p_identifier).toBe(
        `${IP}|${scopeHash('ORD-GET')}`,
      );
    });
    ...
    it('polling do MESMO pedido não consome o teto por IP', async () => {
      ...
      rpcResult = {
        kind: 'resolve',
        value: rpcRow({ allowed: true, hitCount: 37, remaining: 563 }),
      };
    ...
    it('só o PRIMEIRO toque em cada pedido novo cobra um ponto do teto por IP', async () => {
      ...
      expect(buckets['verify-payment-scan'][0].p_limit).toBe(
        RATE_LIMITS.verifyPayment.distinctScope.limit,
      );
    });
    ...
    it('compradores diferentes no mesmo IP não dividem o orçamento de polling', async () => {
      rpcResult = {
        kind: 'resolve',
        value: rpcRow({ allowed: true, hitCount: 90, remaining: 510 }),
      };
    ...
    it('o balde do pedido ainda corta abuso concentrado num único order_code', async () => {
      rpcResult = {
        kind: 'resolve',
        value: rpcRow({ allowed: false, hitCount: 601, remaining: 0, retryAfter: 120 }),
      };
      const res = createMockRes();
  ...
  describe('customer-login: escopo por e-mail + teto de varredura por IP', () => {
    ...
    it('usa (IP + hash do e-mail) como chave, nunca o e-mail em claro', async () => {
      await enforceRateLimit(
        loginReq('professora@escola.com'),
        createMockRes(),
        RATE_LIMITS.customerLogin,
      );
    ...
    it('o balde por conta ainda corta força bruta concentrada num único e-mail', async () => {
      rpcResult = {
        kind: 'resolve',
        value: rpcRow({ allowed: false, hitCount: 6, remaining: 0, retryAfter: 600 }),
      };
      const res = createMockRes();

      const gate = await enforceRateLimit(
        loginReq('alvo@escola.com'),
        res,
        RATE_LIMITS.customerLogin,
      );
