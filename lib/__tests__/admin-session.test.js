...
// `res` completo: `ensureAdminSession` RESPONDE, não devolve descrição de
// resposta. É o corpo emitido que interessa ao item P0.1.
function createRespondingRes() {
  const res = {
    statusCode: 0,
    body: undefined,
    headers: {},
...
describe('admin-session', () => {
  ...
  // ── P0.1: o re-login do admin ────────────────────────────────────
  // Estas duas respostas eram o único lugar do repositório que nunca migrou
  // para `fail()`. Sem `code`, o cliente comparava TEXTO em português e o
  // `includes('sessao admin')` (sem acento) nunca casava com
  // 'Sessão admin inválida ou expirada.' (com acento) — `onAuthExpired()`
  // não disparava e a dona da loja via um erro genérico.
  describe('ensureAdminSession (envelope da regra A1/A2)', () => {
    it('sem cookie: 401 com code ADMIN_SESSION_INVALID', () => {
      const res = createRespondingRes();
      const allowed = ensureAdminSession({ method: 'GET', headers: {} }, res);

      expect(allowed).toBe(false);
      expect(res.statusCode).toBe(401);
      expect(res.body.success).toBe(false);
      expect(res.body.error.code).toBe('ADMIN_SESSION_INVALID');
      expect(res.body.error.message).toBe('Sessão admin inválida ou expirada.');
    });

    it('nunca devolve `error` como string (formato extinto)', () => {
      const res = createRespondingRes();
      ensureAdminSession({ method: 'GET', headers: {} }, res);
      expect(typeof res.body.error).toBe('object');
    });

    it('sessão válida em método de escrita sem Origin: 403 com code FORBIDDEN', () => {
      const { req } = issueSession('dona@loja.com');
      const res = createRespondingRes();
      const allowed = ensureAdminSession({ ...req, method: 'POST' }, res);
    ...
    it('sessão válida em leitura: deixa passar sem responder', () => {
      const { req } = issueSession('dona@loja.com');
      const res = createRespondingRes();

      expect(ensureAdminSession({ ...req, method: 'GET' }, res)).toBe(true);
