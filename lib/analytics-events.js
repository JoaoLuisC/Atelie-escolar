...
function isPiiKey(key) {
  ...
  return ['email', 'cpf', 'phone', 'telefone', 'password', 'senha'].some((term) => k.includes(term));
}
...
function sanitizeValue(value, depth) {
  ...
  if (Array.isArray(value)) {
    ...
    return value.slice(0, MAX_PROP_KEYS).map((item) => sanitizeValue(item, depth + 1)).filter((v) => v !== undefined);
  }
