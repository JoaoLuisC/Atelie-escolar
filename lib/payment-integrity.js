...
// handlers/verify-payment.js (polling do frontend). A correção do P0-1 foi
...
//
// Os testes de paridade em handlers/__tests__/payment-integrity.test.js continuam
...
// • TOLERÂNCIA de 1 centavo: `applyDiscountToItems` (handlers/create-payment.js)
...
// ── A TOLERÂNCIA DE 1 CENTAVO, E POR QUE ELA CONTINUA AQUI ───────────
// A justificativa ORIGINAL desta folga era interna: `applyDiscountToItems`
// (handlers/create-payment.js) rateava o desconto do cupom escalando preços por um
// fator fracionário e absorvia a sobra no último item elegível, então a soma da
// preference podia diferir do total do pedido por frações de centavo. Ou seja:
// tolerávamos o NOSSO próprio drift na porta que entrega o produto pago.
//
// Essa causa foi eliminada na regra E3 (13/08/2026). O rateio agora acontece em
// centavos inteiros (lib/money.js) e `orders.total_amount` é DERIVADO da soma
// real dos itens cobrados — os dois números são iguais por construção, não por
// arredondamento feliz. O invariante está afirmado em
// handlers/__tests__/payment-integrity.test.js.
//
// A folga NÃO foi zerada junto, e a razão é assimetria de custo: se sobrar
// qualquer fonte de diferença de 1 centavo do lado do gateway, zerar produz
// falso NEGATIVO — recusar quem pagou certo. E um pedido recusado no webhook
// não é reprocessado: vira cliente que pagou e não recebe. Com a folga, o pior
// caso é entregar por 1 centavo a menos, o que é irrelevante.
//
// Para zerar com segurança: observar `payment_amount_mismatch` em produção por
// um período com volume real; se nenhum evento tiver `shortfallCents === 1`,
// baixar para 0 e manter os testes de borda.
...
function checkPaymentIntegrity(payment, order) {
  ...
  // handlers/create-payment.js:346 calcula `Math.max(0, subtotal - desconto)` e
