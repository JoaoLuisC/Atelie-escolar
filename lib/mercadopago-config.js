...
const log = createLogger('mercadopago-config');
...
//   handlers/webhook.js: (a) a transição de estado é idempotente e atômica —
...
function readPositiveSeconds(envName, fallback, maximum) {
  ...
  if (maximum && parsed > maximum) {
    log.warn('env_acima_do_teto', { envName, valor: parsed, teto: maximum });
    return maximum;
  }
  return parsed;
}
...
function resolveFutureSkewSeconds(toleranceSeconds) {
  const skew = readPositiveSeconds(
...
function inspectWebhookSignature(req) {
  ...
  if (aged) {
    // Rajada destas linhas com o MESMO payment_id é o padrão de replay; uma
    // linha isolada é reentrega legítima. Com os campos separados, distinguir
    // os dois vira um agrupamento por payment_id em vez de leitura de texto.
    log.warn('webhook_assinatura_valida_ts_antigo', {
      ageSeconds,
      toleranceSeconds,
      paymentId,
      requestId: xRequestId,
      desfecho: 'aceito_como_reentrega',
    });
  }
