...
const log = createLogger('security-logger');
...
async function postExternalAlert(payload) {
  ...
  try {
    ...
    log.warn('alerta_externo_falhou', { reason: err.message });
  }
}
...
async function recordSecurityEvent({
  ...
  //
  // Passa por lib/logger.js (regra F1) e não por um `console.warn` próprio: era
  // o único ponto do backend que ainda montava a própria linha de log. Manter
  // um segundo emissor significaria que a redação de campos sensíveis e o
  // filtro de nível valem para 78 chamadas e não valem justamente para a que
  // registra evento de segurança. O `severity` do evento vira o `level` do log,
  // que é o mapeamento que já existia.
  const emitAt = safeSeverity === 'error' ? log.error : log.warn;
  emitAt(record.event, {
