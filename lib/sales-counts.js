...
function addQuantity(counts, productId, rawQuantity) {
  ...
  counts.set(key, (counts.get(key) || 0) + (Number.isFinite(quantity) && quantity > 0 ? quantity : 0));
}
 ...
 * (api/create-payment.js insere os dois em sequência), então filtrar os itens
...
async function loadSoldCountByProduct() {
  ...
  try {
    ...
    console.warn('[sales-counts] resposta inesperada do RPC product_sales_counts; usando varredura limitada.');
  } catch (error) {
    console.warn('[sales-counts] RPC product_sales_counts indisponível, usando varredura limitada:', error.message);
  }

  try {
    return await loadViaScan(sinceIso);
  } catch (error) {
    console.warn('[sales-counts] varredura de vendas falhou; vitrine segue sem ranking:', error.message);
    return new Map();
  }
}
