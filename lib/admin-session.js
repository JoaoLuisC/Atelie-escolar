...
const { resolveSecret, shouldUseSecureCookie, isLocalDevOrTest } = require('./env-secret');
...
function createSessionToken(identity = {}) {
  ...
  const payload = {
    ...
    email: String(identity?.email || '').trim().toLowerCase() || null,
    role: String(identity?.role || '').trim().toLowerCase() || null,
    iat: now,
...
function getAdminIdentity(req) {
  ...
  return String(parsed.payload?.email || '').trim().toLowerCase() || 'admin';
}
...
function ensureAdminSession(req, res) {
  ...
  if (STATE_CHANGING_METHODS.has(String(req?.method || '').toUpperCase()) && !isSameOriginRequest(req)) {
    res.status(403).json({ success: false, error: 'Origem não permitida (possível CSRF).' });
    return false;
  }
